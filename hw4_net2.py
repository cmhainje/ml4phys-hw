...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim=8, n_features=8575):
        ...
        m = n_features
        Tkw = dict(padding=1)
        Ckw = dict(padding='same', padding_mode='circular')
        
        layers = []
        layers.append(nn.Linear(latent_dim, channels[0] * m))
        layers.append(nn.Unflatten(1, (channels[0], m)))
        for i in range(len(channels) - 1):
            # layers.append(nn.ConvTranspose1d(channels[i], channels[i + 1], 3, **Tkw))
            # layers.append(nn.ReLU())
            layers.append(nn.Conv1d(channels[i], channels[i + 1], 3, **Ckw))
            if i != len(channels) - 2:
                layers.append(nn.ReLU())
        self.cnn = nn.Sequential(*layers)
